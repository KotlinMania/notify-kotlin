...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Notify.swiftmodule/`
...
//      `import Notify` above would fail at compile time.
//
//   2. The static archive `libNotify.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Notify" }` and
//      `flattenPackage = "io.github.kotlinmania.notify"` configuration in
...
final class NotifyExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Notify swift module imported cleanly")
    }
}
