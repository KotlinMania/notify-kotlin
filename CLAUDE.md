# Claude Code Project Instructions — notify-kotlin

## Project Overview

This is **notify-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`notify`](https://crates.io/crates/notify).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/notify/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/notify>
