...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`notify`](https://crates.io/crates/notify).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `notify` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Cross-platform filesystem notification watcher
...
Original work copyrighted by the upstream `notify` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
